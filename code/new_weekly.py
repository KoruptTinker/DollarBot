...
def create_original_line_chart(df, userid):
    ...
    plt.xticks(grouped_data.index, grouped_data['Year'].astype(str) + '-' + grouped_data['Week'], rotation=45)
...
def create_bar_chart(df, userid):
    grouped_data = df.groupby(['Year', 'Week']).agg({'Cost': 'sum'}).reset_index()
    fig = px.bar(grouped_data, x='Week', y='Cost', color='Year', barmode='group',
...
def create_pie_chart(df, userid):
    ...
    fig = px.pie(category_data, names='Category', values='Cost', 
                 title='Category-wise Spending Distribution', hole=0.4)
