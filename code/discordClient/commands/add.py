...
async def add(
            ...
            try:
                ...
                if user_data is None:
                    await interaction.response.send_message("Error: Please connect your telegram account to proceed")
                    return
                expense_date = datetime.strptime(date, "%Y-%m-%d")
                if expense_date > datetime.now():
                    await interaction.response.send_message("Error: Date cannot be in the future!", ephemeral=True)
                    return
            except ValueError:
                await interaction.response.send_message("Error: Invalid date format. Use YYYY-MM-DD", ephemeral=True)
                return

            # Validate amount
            if amount <= 0:
                await interaction.response.send_message("Error: Amount must be greater than 0!", ephemeral=True)
                return
...
async def setup(tree: app_commands.CommandTree):
    tree.command(name="add", description="Add a new expenditure")(
        add
    )
