    ...
    def cal(c):
        ...
        if not result and key:
            ...
            if result > data:
                bot.send_message(
                    chat_id,
                    "Cannot select future dates. Please try /add again with a valid date.",
                )
            else:
...
def post_category_selection(message, bot, date):
    ...
    try:
        ...
        if selected_category not in helper.getSpendCategories():
            ...
            raise Exception(
                f'Sorry, I don\'t recognize this category "{selected_category}"!'
            )
        ...
        bot.register_next_step_handler(
            msg, post_currency_selection, bot, selected_category, date
        )
    except Exception as e:
        logging.exception(str(e))
        bot.reply_to(message, "Oh no! " + str(e))
        display_text = "\n".join(
            f"/{c}: {desc}" for c, desc in helper.getCommands().items()
        )
...
def post_currency_selection(message, bot, selected_category, date):
    ...
    bot.register_next_step_handler(
        msg, post_amount_input, bot, selected_category, selected_currency, date
    )
